...
import { Menu, Calendar, PanelLeftClose, PanelLeft } from "lucide-react";
...
const ROUTE_CONFIGS: RouteHeaderMeta[] = [
  ...
  },
  {
    prefix: "/profile",
    title: "My Profile",
    description: "Manage your personal information, profile photo, and security credentials.",
    category: "Account",
  },
];
...
export function TopHeader() {
  ...
  return (
    <header className="border-b border-border/60 sticky top-0 bg-background/85 backdrop-blur-md z-30 transition-all duration-150">
      <div className="page-shell !py-2 min-h-16 flex items-center justify-between">
        {/* Left: Desktop Sidebar Toggle, Mobile Menu & Page Meta */}
        <div className="flex items-center gap-3 min-w-0 flex-1 py-1">
          {/* Mobile Drawer Trigger */}
          <Button
            variant="ghost"
            size="icon"
            className="md:hidden shrink-0 -ml-1.5 h-9 w-9 text-muted-foreground hover:text-foreground hover:bg-muted"
          ...
          <Button
            variant="ghost"
            size="icon"
            className="hidden md:inline-flex shrink-0 -ml-1.5 h-8 w-8 text-muted-foreground hover:text-foreground hover:bg-muted/70 rounded-lg transition-colors"
