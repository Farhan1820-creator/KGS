...
export async function syncTeacherEmployees(): Promise<ActionResult<Record<string, never>>> {
  try {
    ...
    for (const t of missing) {
      const subjectNames = (t.teacherSubjects && t.teacherSubjects.length > 0)
        ? t.teacherSubjects.map((ts) => ts.subject.name)
        : t.subject
        ? [t.subject.name]
        : [];

      const designation = subjectNames.length > 0 ? `${subjectNames.join(", ")} Teacher` : "Teacher";
